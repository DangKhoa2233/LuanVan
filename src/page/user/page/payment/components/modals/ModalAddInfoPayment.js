...
const ModalAddInfoPayment = ({ open, onOpen, title, data }) => {
  ...
  const method = useForm({
    ...
    resolver: yupResolver(
      yup.object().shape({
        hoTen: yup.string().required("Yêu cầu nhập họ tên vào"),
        soDt: yup
          .number()
          .required("Yêu cầu nhập số điện thoại vào")
          .typeError("Yêu cầu nhập số điện thoại vào"),
        diaChi: yup.string().required("Yêu cầu nhập vào"),
      })
    ),
  });
