...
const ModalOrderDetail = ({ open, onOpen, title, data, fetcher }) => {
  ...
  const renderListOrder = () => {
    return data?.danhSach?.map((sach, index) => {
      return (
        <div className="grid grid-cols-7 gap-[10px]" key={index}>
          <img
          ...
          <p>
            Thành tiền:{" "}
            {(
              parseInt(sach?.sach?.tienCoc) * parseInt(sach?.soLuong) +
              sach?.giaThue * parseInt(sach?.soLuong)
            )?.toLocaleString()}
          </p>
                ...
                <>
                  {data?.tinhTrang != 6 && (
                    <>
                      ...
                      <span>
                        {data?.thongTinGiaoHang?.ngayNhanHangDuKien
                          ?.ngayBatDau +
                          " - " +
                          data?.thongTinGiaoHang?.ngayNhanHangDuKien
                            ?.ngayKetThuc}
